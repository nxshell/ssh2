...
const IGNORE_PAYLOAD = Buffer.from([
  MESSAGE.IGNORE,
  // "keepalive@openssh.com"
  0, 0, 0, 21,
    107, 101, 101, 112, 97, 108, 105, 118, 101, 64, 111, 112, 101, 110, 115,
...
class Protocol {
  ...
  ignore() {
    const p = this._packetRW.write.allocStart;
    const packet = this._packetRW.write.alloc(IGNORE_PAYLOAD.length);

    packet.set(IGNORE_PAYLOAD, p);

    this._debug && this._debug(
      'Outbound: Sending ignore (IGNORE: keepalive@openssh.com)'
    );
    sendPacket(this, this._packetRW.write.finalize(packet));
  }
  rekey() {
